...
local function spawnCowPed(model, coords, heading, cow)
    ...
    if DoesEntityExist(cowPed) then
        ...
        Wait(100)
        if not FarmingTargetAddLocalEntity(cowPed, {
            {
                name = 'cb_farming_milk_cow',
                label = "Milk Cow",
            ...
            lib.print.warn('[cb-farming] No target resource (ox_target / qb-target); cows have no milk interaction.')
        end
    else
...
local COW_TARGET_OPTIONS_META = {
    { name = 'cb_farming_milk_cow', label = 'Milk Cow' },
}

function DeleteCows()
    for _, cowPed in ipairs(spawnedCows) do
        if DoesEntityExist(cowPed) then
            FarmingTargetRemoveLocalEntity(cowPed, COW_TARGET_OPTIONS_META)
            DeleteEntity(cowPed)
        end
    end
    spawnedCows = {}
